...
//        floorGeometry.addControl(new RigidBodyControl(new PlaneCollisionShape(plane), 0));
		...
		for (int i = 0; i < 5; i++) {
			...
			// RigidBodyControl automatically uses Sphere collision shapes when attached to
			// single geometry with sphere mesh
	...
	/**
	 * creates the necessary input listener and action to shoot balls from the
	 * camera
	 *
		...
		ActionListener actionListener = new ActionListener() {
			...
			public void onAction(String name, boolean keyPressed, float tpf) {
				...
				if (name.equals("shoot") && !keyPressed) {
					...
					//bulletGeometry.setShadowMode(ShadowMode.InPass);
	 ...
	 * RigidBodyControl's collision shape. Surface has four slightly concave corners
	 * to allow for multiple tests and minimize falling off the edge of the floor.
	 *
	 * @param assetManager    for loading assets
